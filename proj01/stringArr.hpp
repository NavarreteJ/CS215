...
class StringArr {
  ...
  int longestDescending(std::string seq[]); // Populate seq with the longest sequence of alphabetically descending strings (A-Z), and return the length of the sequence
        

    private:
        /* Whatever you use to store data, along with any helper functions,
 *         go here */
        std::string arr[MAXSIZE];
	int length;
};
